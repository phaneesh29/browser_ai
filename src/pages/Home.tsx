...
export default function Home() {
  ...
  // Move the WebGPU diagnostic check here to keep App.tsx clean
  useEffect(() => {
    setPageTitle('Home')
  ...
  const copyCode = () => {
    navigator.clipboard.writeText(`import { CreateWebWorkerMLCEngine } from '@mlc-ai/web-llm';

// Initialize a WebGPU accelerated local LLM engine in a Web Worker
const engine = await CreateWebWorkerMLCEngine(
  new Worker(new URL('./worker.ts', import.meta.url), { type: 'module' }),
  'Llama-3-8B-Instruct-q4f16_1-MLC'
);

// Stream inference response directly in user browser tab
const chunks = await engine.chat.completions.create({
  messages: [{ role: 'user', content: 'Define client-side intelligence.' }],
  stream: true
});
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 text-slate-900 selection:bg-blue-600/10 selection:text-blue-700">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-6 py-12 flex flex-col box-border space-y-24">
        ...
        {/* Value Props */}
        <section id="features" className="max-w-5xl mx-auto px-4 scroll-mt-24">
        ...
        {/* Integration snippet */}
        <section id="integration" className="max-w-4xl mx-auto px-4 scroll-mt-24">
          <div className="text-center mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900">Developer Integration</h2>
            <p className="text-slate-500 mt-2">Initialize models on target clients using standard MLC Web LLM bindings.</p>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 font-mono text-xs sm:text-sm text-slate-300 leading-relaxed shadow-sm relative group">
            <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-800">
              ...
              <button
                onClick={copyCode}
                className="text-[10px] text-slate-505 hover:text-white uppercase tracking-wider font-semibold bg-slate-800/40 px-2.5 py-1 rounded transition-colors cursor-pointer"
              >
